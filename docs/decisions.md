...
Decisions from the plan review of 2026-09-22 were folded into the entries they amend instead of
being appended as amendment trails; nothing had shipped, so the history lives in git.
...
- **start / stop** a VM or the instance — provider runtime call.
- **reset** a VM or the instance — revert to the golden snapshot. Fast, keeps plugin state.
...
under the hood (Windows Event Forwarding, for example) is out.
...
**Considered alternative: libvirt-python for create/destroy too.** The smoke path (#1) already
creates a domain, network, volume and cloud-init seed with raw libvirt-python, and the provider's
...
**Kill criterion.** OpenTofu is dropped in favour of libvirt-python if either: #11 Q6 cannot keep
`tofu plan` clean with the snapshot method that #11 Q4 selects for golden-state correctness
(`lifecycle.ignore_changes` on the affected attributes is an acceptable way to keep it clean), or
#3 slice 1 finds the OpenTofu module duplicating the smoke path's XML generation. **The snapshot
method is chosen for correctness of golden state, never to satisfy OpenTofu.**
...
snapshots and disk contents; the module ignores power state. OpenTofu is never run against a
`ready` instance except to destroy it, and `destroy_instance` deletes the golden snapshot before
...
The "small clarifications" that lived here were folded into D15 (schema clarifications) and the
roadmap's out-of-scope paragraph (attacker seat; HCP Vagrant dates) on 2026-09-22. The number is
kept so old references resolve.
...
2. The **build orchestrator**: the code that drives an instance through D3's states —
   `create_instance` → wait for SSH → credential swap (D20) → *plugin application hook* → golden
   snapshot → `ready` — plus `destroy` and `rebuild`, and resume from any non-`ready` state.
...
both and names the instance after the config. `destroy` removes the VMs and the record; `rebuild`
is destroy plus build with the same record.
...
per-instance lock (one mutating operation per instance at a time); the job runner (#6) bounds
...
- **Static addressing.** A node's network attachment is `{network, ip}` with `ip` optional,
  ...
  fixture CIDR must not collide with libvirt's stock `default` network (192.168.122.0/24); libvirt
  refuses to start a network whose subnet overlaps one already on the host.
...
problem; the second needs a deliverable, and until now no epic owned it.

**Consequences.** On the KVM host the API listens on a LAN address that students reach, so the
guest-to-host firewall rule from D1 is mandatory there, not optional; #8's student-access slice
delivers it.
...
would mean two revert procedures to test and keep working.
...
like the others. Preflight requires an EFI firmware descriptor.
...
runs, the controller generates a **per-instance SSH keypair**, installs its public key on every
guest (`administrators_authorized_keys` on Windows, `authorized_keys` on Linux), sets a
per-instance local administrator password, and removes the build key. The per-instance private
key and passwords are controller state in SQLite (D7), never config. Plugin application (D4) and
every later controller action use the per-instance key. All generated user passwords derive from
the secret seed (D15). The API (#6) exposes a **student-credentials** response for an owned
instance: the student seat's login and the workstation's local administrator password, never the
seed and never the SSH key.
...
Disk-budget checks count overlay growth, not image size.
